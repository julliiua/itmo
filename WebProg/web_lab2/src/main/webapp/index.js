...
canvas.addEventListener("click", function(e) {
    ...
    const mathX = ((e.clientX - rect.left - 200) / 40).toFixed(2);
    const mathY = ((200 - (e.clientY - rect.top)) / 40).toFixed(2);

    // Устанавливаем X и Y в форму
    const xValues = [-5,-4,-3,-2,-1,0,1,2,3];
    let closestX = xValues.reduce((prev, curr) => Math.abs(curr - mathX) < Math.abs(prev - mathX) ? curr : prev);
    document.querySelector(`input[name="x"][value="${closestX}"]`).checked = true;
    document.getElementById("y").value = mathY;
